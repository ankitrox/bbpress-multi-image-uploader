...
if( !function_exists('bbp_uploader_init') ) {
	...
	function bbp_uploader_init() {
		...
		bbp_uploader()->plupload_args =  array(
					...
					'filters'             => array( array('title' => __('Image Files', 'bbpress-multi-image-uploader'), 'extensions' => 'jpg,gif,png,csv') ),
					'multipart'           => true,
...
if( !function_exists('bbp_uploader_post_children') ) {
	...
	function bbp_uploader_post_children( $parent_post ) {
		...
		if( is_numeric($parent_post) )
			return get_children( array('post_parent'=>$parent_post, 'post_type'=>'attachment', 'post_mime_type'=>'image', 'orderby'=>'title', 'order'=>'ASC' , 'numberposts' => -1 ) );
	}
